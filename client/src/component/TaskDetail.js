...
const TaskDetail = ({ item, checkManager, close, isOpen, update1 }) => {
  ...
  const fetchTaskComment = async function () {
    try {
      const fetchCMT_res = await axios.get(`${process.env.REACT_APP_SERVER}/task_comments/comments-in-task`, {
                ...
                <div key={index} className="flex gap-4 items-start">
                  ...
                  <div>
                    <h3 className="font-semibold my-1">  {
                      joinedFromBackend.find(item => item.id == obj.user_id)?.name || "Member"
                    }</h3>
