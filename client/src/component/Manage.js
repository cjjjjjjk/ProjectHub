...
const Manage = ({ task, model }) => {
  ...
  return (
    <div className="flex flex-col p-8 gap-x-8">
      ...
      <div className="flex flex-col bg-slate-200 gap-y-4 p-2 overflow-auto mt-4 ">
        {sortedTasks.map((item) => (
          <div key={item.id} className="flex flex-row gap-x-4 bg-white p-2 rounded-md">
            ...
            <div className="w-1/5 text-sm">{item.type}</div>
            <div className={`w-[4.5rem] p-1 h-6 text-xs text-white ${getPriority(item.priority)} rounded-md`}>{getPriorityName(item.priority)}</div>
            <div className="w-1/3 whitespace-nowrap">{item.description}</div>
          </div>
        ))}
